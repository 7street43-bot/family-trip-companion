-- J1B: additive, empty, fail-closed Journal Core schema.
...
create table public.journal_blocks (
  ...
  updated_at timestamptz not null default now(),
  deleted_at timestamptz,
  constraint journal_blocks_entry_workspace_fk foreign key (entry_id, workspace_id) references public.journal_entries(id, workspace_id) on delete cascade,
...
create table public.journal_media (
  ...
  updated_at timestamptz not null default now(),
  deleted_at timestamptz,
  constraint journal_media_entry_workspace_fk foreign key (entry_id, workspace_id) references public.journal_entries(id, workspace_id) on delete cascade,
  constraint journal_media_block_entry_workspace_fk foreign key (block_id, entry_id, workspace_id) references public.journal_blocks(id, entry_id, workspace_id),
...
create index journal_entries_workspace_date_idx on public.journal_entries (workspace_id, entry_date desc, updated_at desc) where deleted_at is null;
create index journal_entries_workspace_updated_idx on public.journal_entries (workspace_id, updated_at desc);
create index journal_entries_tags_gin_idx on public.journal_entries using gin (tags);
create index journal_blocks_entry_sort_idx on public.journal_blocks (workspace_id, entry_id, sort_order, created_at) where deleted_at is null;
create index journal_media_entry_sort_idx on public.journal_media (workspace_id, entry_id, sort_order, created_at) where deleted_at is null;
create index journal_media_block_idx on public.journal_media (workspace_id, block_id) where block_id is not null and deleted_at is null;
create index journal_revisions_target_idx on public.journal_revisions (workspace_id, target_type, target_id, created_at desc);
create index journal_revisions_entry_idx on public.journal_revisions (workspace_id, entry_id, created_at desc);
...
create policy "journal members can read entries" on public.journal_entries for select to authenticated using (private.is_family_workspace_member(workspace_id));
create policy "journal members can read blocks" on public.journal_blocks for select to authenticated using (private.is_family_workspace_member(workspace_id));
...
create policy "journal members can read revisions" on public.journal_revisions for select to authenticated using (private.is_family_workspace_member(workspace_id));
