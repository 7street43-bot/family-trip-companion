...
function fakeRouteBody(keys = []) {
  ...
  const legs = [
    ...
    { fromKey:b, toKey:'origin', fromTitle:'B 景點', toTitle:'家', durationSeconds:1200, distanceMeters:12000 }
  ];
  return {
...
async function run(viewport, label) {
  ...
  assert.match(rowText[1], /10:55 離開/, `${label}: second departure missing`);
  ...
  assert.equal(saved?.scheduleSchemaVersion, 1, `${label}: schedule schema not activated`);
  ...
  assert.equal(saved?.schedulePlan?.returnTime, '11:15', `${label}: persisted return ETA mismatch`);
  assert.deepEqual(saved?.schedulePlan?.stopKeys, ['place:A','place:B'], `${label}: schedule stop order mismatch`);
  assert.equal(saved?.schedulePlan?.stops?.[0]?.durationMinutes, 60, `${label}: first dwell not persisted`);
  assert.equal(saved?.schedulePlan?.stops?.[1]?.durationMinutes, 30, `${label}: second dwell not persisted`);
