...
function ensureDemo(){if(!isFixedPreview()||document.getElementById('journalOverlay')?.hidden)return;const left=document.querySelector('#journalOverlay .journal-left');if(!left)return;if(root())return;const el=document.createElement('section');el.id='journalPreviewMediaDemo';el.className='journal-media-section journal-preview-media-demo';const anchor=left.querySelector('.journal-alert');if(anchor)anchor.insertAdjacentElement('afterend',el);else left.prepend(el);render();}
