...
async function saveCaption(button){const id=selectedEntryId(),mediaId=String(button.dataset.mediaCaptionSave||''),version=Number(button.dataset.version),input=section()?.querySelector(`[data-media-caption-input="${CSS.escape(mediaId)}"]`);if(!id||!mediaId||!input)return;const s=await status();if(!s.originMatch)return statusMessage('Preview 安全模式不修改正式照片。','error');state.loading=true;renderSection();try{const r=await journalBinding.updateMedia(id,mediaId,version,{caption:String(input.value||'').trim()||null});if(r?.status==='conflict')statusMessage('照片版本已更新，已重新載入，請再確認一次。','error');else statusMessage('照片說明已更新。','good');}catch(err){statusMessage(err?.message||'更新照片說明失敗。','error');}finally{state.loading=false;await loadAlbum(id);}}
