...
function closeJournalForAppNav(target){const btn=target?.closest?.('#bottomNav .nav-item');const n=nav(),o=overlay();if(!btn||btn===n||!o||o.hidden)return false;state.previous=null;const close=o.querySelector('[data-journal-close]');if(!close)return false;close.click();return true;}

function mount(){if(state.mounted)return;state.mounted=true;const n=nav();if(!n)return;n.disabled=false;n.addEventListener('click',async ev=>{ev.preventDefault();ev.stopPropagation();n.setAttribute('aria-busy','true');try{if(!(await openJournal())){n.dataset.error='journal-not-ready';console.error('Journal UI did not become ready within 2 seconds.');}}finally{n.removeAttribute('aria-busy');}});const bottom=document.getElementById('bottomNav');bottom?.addEventListener('click',ev=>{closeJournalForAppNav(ev.target);},true);const observer=new MutationObserver(()=>{syncPending();syncOverlay();});observer.observe(document.body,{childList:true,subtree:true,attributes:true,attributeFilter:['hidden','data-pending']});syncPending();syncOverlay();}
...
export const __test={setJournalActive,openJournal,waitForFab,syncPending,closeJournalForAppNav};
